...
export function PortfolioClient({ bets }: Props) {
  ...
  return (
    <div className="flex flex-col gap-6">
      <h1 className="text-2xl font-bold text-[#111827]">{t('Portföy', 'Portfolio')}</h1>
        ...
        {[
          ...
          <div key={s.label} className="bg-white border border-[#E5E7EB] rounded-xl p-4">
            ...
            <p className={`text-lg font-bold mt-1 ${
              s.isProfit ? ((s.profit ?? 0) >= 0 ? 'text-[#16A34A]' : 'text-red-500') : 'text-[#111827]'
            }`}>{s.value}</p>
          </div>
        ))}
      </div>
        ...
        {FILTERS.map((f) => (
          <button key={f.key} onClick={() => setFilter(f.key)}
            className={cn('px-3 py-1.5 text-xs font-semibold rounded-lg border transition-colors',
              filter === f.key ? 'border-[#16A34A] text-[#16A34A] bg-[#F0FDF4]' : 'border-[#E5E7EB] text-[#6B7280] hover:border-[#D1D5DB] bg-white'
            )}>
            {f.label}
          </button>
        ))}
      ...
      {/* Bets list */}
      {filtered.length === 0 ? (
        <div className="text-center py-20 text-[#9CA3AF] text-sm">{t('Henüz bahis yok.', 'No bets yet.')}</div>
      ) : (
          ...
          {filtered.map((bet) => {
            ...
            return (
              <Link key={bet.id} href={`/markets/${bet.market_id}`}>
                <div className="bg-white border border-[#E5E7EB] rounded-xl p-4 flex flex-col sm:flex-row sm:items-center gap-3 hover:border-[#D1D5DB] hover:shadow-sm transition-all">
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-[#111827] leading-snug line-clamp-1">{title}</p>
                    <p className="text-xs text-[#9CA3AF] mt-0.5">{formatDate(bet.created_at, lang)}</p>
                  </div>
                  <div className="flex items-center gap-3 shrink-0 text-sm">
                    ...
                    <span className="text-[#9CA3AF]">{bet.odds_at_bet}x</span>
                    ...
                    <span className={cn('text-xs font-semibold px-2 py-0.5 rounded-md',
                      ...
                      bet.status === 'pending' && 'bg-[#F9FAFB] text-[#9CA3AF]',
                    )}>
